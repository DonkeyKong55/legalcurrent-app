...
// Remove the loader export - this is what was causing the build error
...
export default function CreateDefaultNotFoundPage() {
    ...
    // Move the loader logic into useEffect
    useEffect(() => {
            ...
            try {
                ...
                const pages = matches
                    ...
                    .map((match) => {
                        ...
                        const displayPath = url.replaceAll('[', '').replaceAll(']', '');
                        const finalDisplayPath = displayPath === '/' ? 'Homepage' : displayPath;
                        return { url, path: finalDisplayPath };
                    });

                setPageData({ path, pages });
            } catch (error) {
    ...
    const handleCreatePage = useCallback(() => {
        if (pageData) {
            const missingPath = pageData.path.replace(/^\//, '');
            window.parent.postMessage(
                {
    ...
    const missingPath = pageData.path.replace(/^\//, '');
    const existingRoutes = pageData.pages.map((page) => ({
        path: page.path,
        url: page.url,
    }));
